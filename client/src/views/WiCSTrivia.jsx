...
export default function WiCSTrivia() {
    const [triviaData, setTriviaData] = useState(null)
        ...
        async function getTriviaQuestion() {
            const url = "http://127.0.0.1:5000/question?amount=4"
                ...
                {triviaData && triviaData.map((item) => {
                    return (
                        <div className='bg-gray-300 opacity-80 p-8 rounded-2xl flex flex-col gap-4' key={item.id}>
                            <h2  className='text-xl font-serif font-semibold'>{triviaData ? item['question'] : 'Loading Question'}</h2>
                            <h2  className='text-l font-serif font-semibold'>{triviaData ? item['answer'] : 'Loading Answer'}</h2>
                        </div>
                    )
                })}
